...
public class UserController {
	...
	public String saveUser(User user, RedirectAttributes redirectattribute,
		...
		redirectattribute.addFlashAttribute("message", "The user has been added successfully.");
		return getRedirectURLtoAffectedUser(user);
	}

	private String getRedirectURLtoAffectedUser(User user) {
		String firstPartOfEmail = user.getEmail().split("@")[0];
		return "redirect:/users/page/1?sortField=id&sorDir=asc&keyword=" + firstPartOfEmail;
	}
	...
	public String editUser(@PathVariable(name = "id") Integer id, Model model, RedirectAttributes redirectattribute) {
		try {
			...
			redirectattribute.addFlashAttribute("message", exp.getMessage());
